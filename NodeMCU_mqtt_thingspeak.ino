...
#define SECRET_SSID "your_ss_id"                            // replace MySSID with your WiFi network name
#define SECRET_PASS "your_password"                     // replace MyPassword with your WiFi password

#define SECRET_CH_ID 111111                       // replace 0000000 with your channel number
#define SECRET_WRITE_APIKEY "13X74ZVGB7SWEOIZ"     // replace XYZ with your channel write API Key
...
void loop() {
  ...
  int httpCode = ThingSpeak.writeField(myChannelNumber, 1, valueA0, myWriteAPIKey);
   ...
   httpCode = ThingSpeak.writeField(myChannelNumber, 2, valueA0, myWriteAPIKey);
   httpCode = ThingSpeak.writeField(myChannelNumber, 3, random(10,20), myWriteAPIKey);
   httpCode = ThingSpeak.writeField(myChannelNumber, 4, random(0,1000), myWriteAPIKey);
   httpCode = ThingSpeak.writeField(myChannelNumber, 5, random(0,10), myWriteAPIKey);
